 ...
 * 1. Retired Faculty individual-response routes redirect to aggregate-only analytics
 *    with no respondent label, email, domain user ID, identified name, or raw rows.
 ...
 * 4. Unrelated Faculty Member sees empty aggregate scope, Program Head assigned to another
 *    Program and guessed deep links receive safe denial (Not Found) without leaking data.
...
test.describe("Cross-role response privacy (§36, §37, §38, §40, #548)", () => {
  test("aggregate-only faculty review, identified program head access, denial, and analytics privacy", async ({
    page,
  }) => {
    const fx = fixture();

    // ── 1. Faculty Aggregate-Only Review ────────────────────────────────────
    // Retired individual-response routes redirect to aggregate analytics;
    // Faculty never receives respondent identity, respondent labels, or raw rows.
    await loginAs(page, fx.demoFaculty.email);
    await page.goto(`/faculty/cilo-evaluations/${fx.courseEvaluation.id}`);
    await expect(page).toHaveURL(/\/faculty\/analytics.*evaluationId=/);
    await expect(page.getByRole("heading", { name: fx.courseEvaluation.title })).toBeVisible();
    await expect(page.getByText("Aggregate evidence only")).toBeVisible();
    await expect(
      page.getByText("Aggregated results for this evaluation across the classes you teach.")
    ).toBeVisible();
    await expect(
    ...
    await expect(page.getByText(/^Respondent R-\d{6}$/)).not.toBeVisible();
    await page.goto(
    ...
    await expect(page.getByText(qualAnswerText, { exact: true })).not.toBeVisible();
    ...
    // Respondent context label renders the full Program name + year + section
    // (e.g., "Bachelor of Science in Information Technology · FOURTH_YEAR · MORNING").
    // .first() avoids strict-mode collisions when the program name also appears in the shell.
    ...
    await page.goto(`/faculty/cilo-evaluations/${fx.courseEvaluation.id}`);
    await expect(page).toHaveURL(/\/faculty\/analytics.*evaluationId=/);
    await expect(page.getByText("No evaluation evidence in this scope")).toBeVisible();
    await expectNoAxeViolations(page);
    await page.goto(
      `/faculty/cilo-evaluations/${fx.courseEvaluation.id}/responses/${fx.courseResponse.id}`
    );

...
export function MobileSidebarDrawer({ roles = [], user }: MobileSidebarDrawerProps) {
  ...
  return (
    <>
      <button ref={triggerRef} type="button" onClick={() => setIsOpen(true)} className={cn("text-text-muted hover:bg-surface-muted hover:text-text-primary flex min-h-11 min-w-11 items-center justify-center rounded-md transition-colors focus-visible:outline-2 focus-visible:outline-ring", dean ? "md:hidden" : "lg:hidden")} aria-label="Open navigation menu" aria-expanded={isOpen}>

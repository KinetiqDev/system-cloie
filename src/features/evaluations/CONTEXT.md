...
Publication resolves a template's CILO and GO question bindings against the questions in its stored `structure`. `TemplateQuestion.type` is the canonical `QuestionType` union and `"likert"` is the only spelling production writes, so a binding keyed by section and question key must resolve against that lowercase value; both the Faculty-owned and on-behalf publication context paths apply the same rule. The publication scanner tolerates the uppercase `type`/`question_type` spellings for compatibility with its earlier predicate and any legacy JSON, and never writes a structure — it only reads one. A binding that resolves to no Likert question is rejected before deployment creation rather than silently dropped.

...
_Avoid_: Program Learning Outcome, Graduate Outcome, college-level GO

**Graduate Outcome (GO)**:
A program-level learning outcome owned by one Academic Program; Program Heads administer GOs within their assigned Program scope.
_Avoid_: Institutional outcome, program-level template
...
A Course owned by one Academic Program; its CILOs align only to Graduate Outcomes of that owning Program.
_Avoid_: General Education course, cross-program course

**Typed outcome mapping**:
An alignment row between a CILO and exactly one target layer: General Education CILO to Institutional Outcome, or Program-specific CILO to owning-Program Graduate Outcome. The Course scope determines the valid relation; wrong-layer writes are rejected.
_Avoid_: Unqualified mapping, CILO-to-GO for General Education, dual-layer mapping
...
_Avoid_: GO-only readiness, per-assignment readiness, CILO count as readiness

**Completed-period readiness snapshot**:
An immutable, versioned record of readiness written when an Academic Period completes; it carries typed target details for new snapshots, while existing snapshots retain their legacy interpretation and are never rewritten by later mapping or catalog changes.
...
The Secretary owns the Institutional Outcome catalog and holds college-wide correction authority for both typed mapping relations and for CILO and GO administration, using exact before/after review, explicit confirmation, freshness recheck, and atomic save.
_Avoid_: Secretary as GO owner, unconfirmed administrative write

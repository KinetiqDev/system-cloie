...
import { z } from "zod";
import { ANALYTICS_INSIGHT_VIEWS } from "./ai-insight-contract";
...
/**
 * The AI Server Action accepts only the selected `programId`, the requested
 * analytics view, and validated tab/filter state. Unknown keys
 * (client-supplied aggregates, comments, or identities) are rejected
 * outright; evidence is never read from the client.
 */
export const aiActionInputSchema = z
  .object({
    programId: z.string().uuid(),
    analyticsView: z.enum(ANALYTICS_INSIGHT_VIEWS),
    filters: z.object({

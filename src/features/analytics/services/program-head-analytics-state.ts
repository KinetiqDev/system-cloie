...
export type AnalyticsFilterState = {
  ...
  evidenceSource?: AnalyticsEvidenceSource;
...
const schema = z.object({
  ...
  evidenceSource: z
    ...
    .optional()
    .catch(undefined),
  stakeholder: z.enum(["STUDENT", "ALUMNI", "INDUSTRY_PARTNER"]).optional().catch(undefined),
  majorId: uuid.optional().catch(undefined),
  yearLevel: z.string().optional().catch(undefined),
...
function couple(
  state: Omit<AnalyticsFilterState, "tab"> & { tab: AnalyticsTab | LegacyAnalyticsTab }
): AnalyticsFilterState {
  let stakeholder = state.stakeholder;
  if (state.evidenceSource === "COURSE") stakeholder = undefined;
  if (
    (state.evidenceSource === "PROGRAM_WIDE_STUDENT" && stakeholder !== "STUDENT") ||
    (state.evidenceSource === "ALUMNI" && stakeholder !== "ALUMNI") ||
    (state.evidenceSource === "INDUSTRY" && stakeholder !== "INDUSTRY_PARTNER")
  )
    stakeholder = undefined;
  const { tab, ...rest } = state;
  return {
    ...rest,
    tab: ANALYTICS_TABS.includes(tab as AnalyticsTab) ? (tab as AnalyticsTab) : "outcomes",
    stakeholder,
  };
}
export function parseAnalyticsSearchParams(raw: Raw = {}): AnalyticsFilterState {
  const parsed = schema.parse({
    ...
    evidenceSource: first(raw.evidenceSource),

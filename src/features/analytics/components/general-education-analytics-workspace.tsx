...
export function GeneralEducationAnalyticsWorkspace({ data, filters }: Props) {
  ...
  const hasOptions = periodOptions.schoolYears.length > 0 || periodOptions.semesters.length > 0 || periodOptions.termInstances.length > 0;
    ...
    <div className="flex flex-col gap-6">
      ...
      {hasOptions ? (
        <Card size="sm">
          ...
          <CardContent>
            <form method="get" action={BASE_PATH} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Select label="School Year" name="schoolYearId" value={filters.schoolYearId ?? ""} options={periodOptions.schoolYears.map((o) => ({ value: o.id, label: o.label }))} blankLabel="All school years" />
              <Select label="Semester" name="semester" value={filters.semester ?? ""} options={periodOptions.semesters} blankLabel="All semesters" />
              <Select label="Academic Term" name="termInstanceId" value={filters.termInstanceId ?? ""} options={periodOptions.termInstances.map((o) => ({ value: o.id, label: o.label }))} blankLabel="All terms" />
              <div className="flex items-end gap-2">
                <button type="submit" className={cn(buttonVariants({ size: "sm" }))}>Apply</button>
                <Link href={resetHref()} className="text-link inline-flex min-h-11 items-center px-3 text-sm underline-offset-3 hover:underline">Reset</Link>
              </div>
            </form>
          </CardContent>
        </Card>
      ...
      {emptyReason === "no-assignments" ? (
        <Empty>
          <EmptyHeader><EmptyMedia variant="icon"><ClipboardList aria-hidden /></EmptyMedia><EmptyTitle>No evaluation assignments</EmptyTitle><EmptyDescription>No General Education evaluation assignments in this scope.</EmptyDescription></EmptyHeader>
          <EmptyContent><Link href={resetHref()} className={resetClass}>View all periods</Link></EmptyContent>
        </Empty>
      ) : emptyReason === "no-submissions" ? (
        <Empty>
          <EmptyHeader><EmptyMedia variant="icon"><Inbox aria-hidden /></EmptyMedia><EmptyTitle>No submitted responses</EmptyTitle><EmptyDescription>Assignments exist, but no submitted responses yet.</EmptyDescription></EmptyHeader>
          <EmptyContent><Link href={resetHref()} className={resetClass}>Clear period filter</Link></EmptyContent>
        </Empty>
      ) : null}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          { label: "Response Rate", value: kpi.responseRate === null ? "—" : `${(kpi.responseRate * 100).toFixed(1)}%` },
          { label: "Rating Count", value: String(kpi.ratingCount) },
          { label: "Mean Rating", value: kpi.meanRating === null ? "—" : kpi.meanRating.toFixed(2) },
        ].map((card) => (
          <Card key={card.label} size="sm"><CardContent className="pt-4"><p className="text-muted-foreground text-xs">{card.label}</p><p className="text-xl font-semibold tabular-nums">{card.value}</p></CardContent></Card>
        ))}
      </div>
      {kpi.evaluationOpportunityCount === 0 ? (
        <p className="text-body-sm text-text-secondary">No denominator exists for this scope — response rate is unavailable.</p>
      ) : null}
      ...
      {courseBreakdowns.length > 0 ? (
        ...
        <Empty><EmptyHeader><EmptyMedia variant="icon"><Inbox aria-hidden /></EmptyMedia><EmptyTitle>No course breakdown</EmptyTitle><EmptyDescription>No Course-bound General Education evidence to break down by course in this scope.</EmptyDescription></EmptyHeader></Empty>
      ) : null}
      ...
      {trends.periods.length > 0 ? (
        ...
        <Empty><EmptyHeader><EmptyMedia variant="icon"><Inbox aria-hidden /></EmptyMedia><EmptyTitle>No trend evidence</EmptyTitle><EmptyDescription>No periods with evidence to chart in this scope.</EmptyDescription></EmptyHeader></Empty>
...
function CourseBreakdownSection({ rows }: { rows: GeneralEducationAnalyticsDTO["courseBreakdowns"] }) {
  ...
  return (
    <section aria-label="Course breakdown" className="space-y-3">
      ...
      <p className="text-body-sm text-text-secondary">Cross-program Course-bound General Education evidence only.</p>
      {ranked.length > 0 ? (
        <>
          ...
          <div role="list" className="flex flex-wrap gap-x-4 gap-y-1.5" aria-label="Chart legend">
            {data.map((e, i) => <span role="listitem" key={e.label} className="flex items-center gap-1.5"><ChartSwatch fill={e.value === null ? "var(--muted)" : chartFill(chartId, i)} /><span className="text-muted-foreground text-xs">{e.label}</span></span>)}
          </div>
          <p className="text-body-sm text-text-secondary">
            {ranked.length === 1 ? `${ranked[0].label}: ${ranked[0].value!.toFixed(2)}.` : `Highest mean: ${ranked[0].label} (${ranked[0].value!.toFixed(2)}). Lowest: ${ranked[ranked.length - 1].label} (${ranked[ranked.length - 1].value!.toFixed(2)}).`}
          </p>
        </>
      ) : null}
      <details>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.courseId}>
                  <TableCell className="font-medium">{r.courseCode} — {r.courseTitle}</TableCell>
                  <TableCell className="tabular-nums">{r.meanRating === null ? "N/A" : r.meanRating.toFixed(2)}</TableCell>
                  <TableCell className="text-right tabular-nums">{r.ratingCount}</TableCell>
                  <TableCell className="text-right tabular-nums">{r.submittedResponseCount}</TableCell>
...
function TrendsSection({ trends }: { trends: GeneralEducationAnalyticsDTO["trends"] }) {
  ...
  const chartable = periods.filter((p): p is typeof p & { meanRating: number } => p.meanRating !== null);
  ...
  if (chartable.length === 0 || !hasDrawableRun) {
    return (
      <section aria-label="Trends" className="space-y-3">
        ...
        <p className="text-body-sm text-text-secondary">No comparable history to draw a trend line in this scope.</p>
        <TrendsTable periods={periods} breaks={breaks} />
      </section>
    );
  }
  const runIndexByLabel = new Map<string, number>();
  runs.forEach((run, idx) => { for (const pt of run) runIndexByLabel.set(pt.periodLabel, idx); });
  ...
  const breakLabels = chartable.filter((p) => breakSet.has(p.periodLabel)).map((p) => p.periodLabel);
  return (
    <section aria-label="Trends" className="space-y-3">
      <h2 className="text-title-sm text-foreground">Trends</h2>
      <div className="border-border h-72 w-full rounded-xl border p-3">
        <ChartContainer id={chartId} role="region" className="aspect-auto h-full w-full">
          <LineChart data={data} margin={{ bottom: 10, left: 0, right: 0, top: 10 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} />
            <XAxis dataKey="periodLabel" tickLine={false} axisLine={false} interval={0} angle={-25} textAnchor="end" height={70} tick={{ fontSize: 12 }} />
            <YAxis domain={[0, "auto"]} tickLine={false} axisLine={false} tick={{ fontSize: 12 }} />
            <Tooltip formatter={(v) => [typeof v === "number" ? v.toFixed(2) : v, "Mean"]} contentStyle={{ borderRadius: "8px", border: "1px solid var(--color-border)", backgroundColor: "var(--color-surface)", fontSize: "13px" }} />
            {breakLabels.map((label) => <ReferenceLine key={label} x={label} stroke="var(--color-border)" strokeDasharray="4 4" />)}
            {runs.map((run, i) => {
              const standalone = run.length < 2;
              const fill = standalone ? "var(--muted)" : chartFill(chartId, i);
              return <Line key={i} type="linear" dataKey={`run${i}`} stroke={fill} strokeWidth={2} dot={{ r: 3, fill, strokeWidth: 0 }} activeDot={{ r: 4 }} isAnimationActive={false} connectNulls={false} />;
            })}
          </LineChart>
        </ChartContainer>
      </div>
      {breaks.length > 0 ? (
        <div className="rounded-lg border border-dashed p-3">
          ...
          <ul className="text-text-secondary mt-1.5 list-disc space-y-1 pl-5 text-sm">
            {breaks.map((b, i) => <li key={i}>{b.fromPeriodLabel} → {b.toPeriodLabel}: {b.reason}</li>)}
          </ul>
        </div>
      ) : null}
      <TrendsTable periods={periods} breaks={breaks} />
...
function TrendsTable({ periods, breaks }: { periods: GeneralEducationAnalyticsDTO["trends"]["periods"]; breaks: GeneralEducationAnalyticsDTO["trends"]["breaks"] }) {
  return (
    <details>
          ...
          <TableBody>
            {periods.map((p) => (
              <TableRow key={p.termInstanceId}>
                ...
                <TableCell className="text-right tabular-nums">{p.meanRating === null ? "N/A" : p.meanRating.toFixed(2)}</TableCell>
                <TableCell className="text-right tabular-nums">{p.ratingCount}</TableCell>
                <TableCell className="text-right tabular-nums">{p.submittedResponseCount}</TableCell>
      ...
      {breaks.length > 0 ? <p className="text-body-sm text-text-secondary mt-2">{breaks.length} comparability break{breaks.length === 1 ? "" : "s"} — values joined only within comparable periods.</p> : null}
    </details>
  );
}
...
function FeedbackSection({ feedback }: { feedback: GeneralEducationAnalyticsDTO["feedback"] }) {
  ...
  if (emptyReason === "no-assignments") {
    return <Empty><EmptyHeader><EmptyMedia variant="icon"><ClipboardList aria-hidden /></EmptyMedia><EmptyTitle>No evaluation assignments</EmptyTitle><EmptyDescription>No General Education evaluation assignments, so no feedback to summarize.</EmptyDescription></EmptyHeader></Empty>;
  }
  if (emptyReason === "no-submissions") {
    return <Empty><EmptyHeader><EmptyMedia variant="icon"><ClipboardList aria-hidden /></EmptyMedia><EmptyTitle>No submitted responses</EmptyTitle><EmptyDescription>Assignments exist, but no submitted responses yet.</EmptyDescription></EmptyHeader><EmptyContent><Link href={resetHref()} className={resetClass}>Clear period filter</Link></EmptyContent></Empty>;
  }
  if (emptyReason === "no-qualitative-evidence") {
    return <Empty><EmptyHeader><EmptyMedia variant="icon"><MessageSquareText aria-hidden /></EmptyMedia><EmptyTitle>No qualitative evidence</EmptyTitle><EmptyDescription>Submitted responses exist, but none include qualitative comments.</EmptyDescription></EmptyHeader></Empty>;
  }
  return (
    <section aria-label="Qualitative feedback" className="flex flex-col gap-4">
      ...
      <p className="text-body-sm text-text-secondary">{sourceLabel} — aggregate tokens only; raw comments are never shown.</p>
      {tokens.length > 0 ? <QualitativeWordCloud title="Frequent terms" tokens={tokens} responseCount={qualitativeResponseCount} /> : (
        <Alert variant="information"><AlertTitle>No tokenizable terms</AlertTitle><AlertDescription>Identifier redaction and filtering left no terms to display.</AlertDescription></Alert>
      )}
      <div className="flex flex-col gap-3">
        <h3 className="text-title-sm text-foreground">Prompt counts</h3>
        <p className="text-body-sm text-text-secondary">{qualitativeItemCount} items from {qualitativeResponseCount} submitted {qualitativeResponseCount === 1 ? "response" : "responses"}</p>
        <div className="border-border overflow-x-auto rounded-lg border">
          <Table aria-label="Exact values: Prompt counts">
            <TableHeader><TableRow><TableHead>Prompt</TableHead><TableHead className="text-right">Items</TableHead><TableHead className="text-right">Responses</TableHead></TableRow></TableHeader>
            <TableBody>
              {promptCounts.map((r) => <TableRow key={`${r.sourceLabel}:${r.promptLabel}`}><TableCell className="font-medium">{r.sourceLabel} — {r.promptLabel}</TableCell><TableCell className="text-right tabular-nums">{r.itemCount}</TableCell><TableCell className="text-right tabular-nums">{r.responseCount}</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </div>
      </div>
      {evidenceEvaluations.length > 0 ? (
        <div className="flex flex-col gap-2">
          ...
          <ul className="flex flex-col gap-1">
            {evidenceEvaluations.map((ev) => <li key={ev.evaluationId} className="text-sm text-muted-foreground">{ev.deploymentName}</li>)}
          </ul>
        </div>
      ) : null}
    </section>

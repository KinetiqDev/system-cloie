        ...
        {rows.length === 0 ? (
          ...
          rows.map((row) => (
            ...
            >
              ...
              <span className="text-label-sm min-w-0 text-right leading-tight">
                <span className="block font-bold tabular-nums">
                  {Math.round((row.completionRate ?? 0) * 100)}%
                </span>
                <span className="text-muted-foreground block break-words tabular-nums">
                  {row.submitted.toLocaleString()} / {row.assigned.toLocaleString()} assignments
                </span>
              </span>
            </Link>
          ))

...
describe("Faculty Course alignment service", () => {
  beforeEach(() => {
    ...
    mocks.go.findMany.mockResolvedValue([{ id: GO_ID, code: "GO-1", description: "Think critically" }]);
    mocks.go.count.mockResolvedValue(1);
  });

  it("returns only the active owning-Program catalog and readiness", async () => {
    const { readFacultyCourseAlignment } = await import(
      "@/features/outcomes/services/manage-faculty-course-alignment"
    );
  ...
  it("prepares an exact multi-CILO diff and rejects a forged cross-Program target", async () => {
    const { prepareCourseAlignmentWrite } = await import(
      "@/features/outcomes/services/manage-faculty-course-alignment"
    );
    ...
    await expect(
      prepareCourseAlignmentWrite({ courseId: COURSE_ID, desired: [{ ciloId: CILO_ID, targetIds: ["55555555-5555-4555-8555-555555555555"] }] })
    ).resolves.toEqual({
      success: false,
  ...
  it("commits the complete diff in one transaction and requires fresh state", async () => {
    const { prepareCourseAlignmentWrite, commitCourseAlignmentWrite } = await import(
      "@/features/outcomes/services/manage-faculty-course-alignment"
    );

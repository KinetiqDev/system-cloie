...
describe("parseAnalyticsSearchParams", () => {
  ...
  it("sanitizes malformed values", () => {
    expect(
      parseAnalyticsSearchParams({ schoolYearId: "bad", termInstanceId: "bad", semester: "bad" })
    ).toEqual({ tab: "outcomes" });
    expect(
      parseAnalyticsSearchParams({ schoolYearId: uuid, semester: "FIRST", termInstanceId: uuid })
    ).toEqual({ tab: "outcomes", schoolYearId: uuid, semester: "FIRST", termInstanceId: uuid });
  });
  it("takes the first non-empty array value", () => {
    expect(parseAnalyticsSearchParams({ tab: ["", "trends"], schoolYearId: ["", uuid] })).toEqual({
      tab: "trends",
      schoolYearId: uuid,
    });
  });
  it("drops individual invalid values while keeping valid neighbors", () => {
    ...
    expect(parseAnalyticsSearchParams({ schoolYearId: "bad", semester: "FIRST" })).toEqual({
      tab: "outcomes",
      semester: "FIRST",
    });
  });
  it("drops incompatible stakeholder values", () => {
    expect(parseAnalyticsSearchParams({ evidenceSource: "COURSE", stakeholder: "ALUMNI" })).toEqual(
      { tab: "outcomes", evidenceSource: "COURSE" }
    );
    expect(
      parseAnalyticsSearchParams({ evidenceSource: "ALUMNI", stakeholder: "STUDENT" })
    ).toEqual({ tab: "outcomes", evidenceSource: "ALUMNI" });
    expect(parseAnalyticsSearchParams({ evidenceSource: "ALUMNI", stakeholder: "ALUMNI" })).toEqual(
...
describe("analytics URLs", () => {
  it("omits the default outcomes tab", () =>
    expect(buildAnalyticsUrl("program-1")).toBe("/program-head/programs/program-1/analytics"));
  it("keeps an explicit outcomes tab for reset links", () =>
    expect(buildAnalyticsUrl("program-1", { tab: "outcomes" })).toBe(
      "/program-head/programs/program-1/analytics?tab=outcomes"
    ));
  ...
  it("serializes filters", () => {
    const url = buildAnalyticsUrl("program-1", {
      tab: "trends",
      schoolYearId: uuid,
      semester: "FIRST",
    ...
    expect(url).toContain("evidenceSource=COURSE");
  });
  it("preserves filters on tab navigation", () =>
    expect(
      buildAnalyticsTabUrl("program-1", "trends", { tab: "outcomes", termInstanceId: uuid })
    ).toBe(`/program-head/programs/program-1/analytics?tab=trends&termInstanceId=${uuid}`));
  it("keeps raw duplicates for canonicalization", () =>
  ...
  it("builds stable scope fingerprints from period and evidence fields", () => {
    ...
    ).toBe(`${uuid}|FIRST|${uuid}|ALUMNI|ALUMNI`);
    expect(buildAnalyticsFilterFingerprint({ evidenceSource: "ALUMNI" })).not.toBe(
      buildAnalyticsFilterFingerprint({ evidenceSource: "COURSE" })
    );
  });
});

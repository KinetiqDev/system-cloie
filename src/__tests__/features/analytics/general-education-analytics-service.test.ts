...
describe("getGeneralEducationAnalytics", () => {
  ...
  it("scopes to Course-bound GE only and submitted only", async () => {
    ...
    expect(where.assignment.course_bound.course_assignment.course.course_scope).toBe(
      "GENERAL_EDUCATION"
    );
    expect(
      prismaMock.evaluationAssignment.count.mock.calls[0][0].where.course_bound.course_assignment
        .course.course_scope
    ).toBe("GENERAL_EDUCATION");
    ...
    prismaMock.quantitativeResponseItem.findMany.mockResolvedValue([
      {
        ...
        section_key: "s",
        item_key: "i1",
        cilo_question_binding: null,
        response: {
          assignment: {
            course_bound: {
              term_instance_id: "t1",
              instrument_version_id: "iv1",
              course_assignment: { course: { id: "c1", code: "GE 101", title: "GE" } },
              instrument: {
                id: "iv1",
                version_number: 1,
                template: { name: "T" },
                ...
                structure_snapshot: [
                  {
                    key: "s",
                    title: "S",
                    items: [{ key: "p", kind: "qualitative", prompt: "Prompt" }],
                  },
                ],
              },
            },
  ...
  it("handles zero opportunities without crashing", async () => {
    const r = await getGeneralEducationAnalytics({
      schoolYearId: "00000000-0000-0000-0000-000000000001",
    });

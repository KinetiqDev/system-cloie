...
`pnpm test` must never write to a hosted database. The four invariant suites below are gated on `RUN_DATABASE_INTEGRATION_TESTS=1` in addition to `DATABASE_URL`:

...
- Five analytics components use raw arrays of 5 to 12 chart colors and no shared shadcn chart wrapper. The existing word cloud is the only non-Recharts visualization and also uses a raw palette. The archived `improve-navigation-rendering-and-caching` change has already narrowed `FacultyAnalyticsData` to aggregate counts and `wordCloudTokens`; the existing serialization suite excludes raw qualitative text. Chart migration preserves this completed privacy contract.
...
- `src/lib/constants/navigation.ts` already has `getDeepestMatchingNavItem` and `getProgramHeadProgramIdFromPathname`; `src/lib/constants/program-head-routes.ts` owns `buildProgramHeadProgramPath` and `PROGRAM_HEAD_ENTRY_PATH`. The canonical Program Head routes are under `src/app/(app)/program-head/programs/[programId]/...`. Existing static Program Head management routes under `src/app/(app)/program-head/` (excluding `profile/` and the root entry page) are compatibility redirects to `/program-head` and are not visual page owners; they are excluded from the migration scope.
- `src/components/ui/` already supplies Base UI primitives for alert dialog, alert, badge, button, card, checkbox, dialog, drawer, dropdown menu, input, label, progress, radio group, select, separator, sheet, skeleton, switch, table, tabs, textarea, toast, and tooltip. The approved registry also provides compatible `chart`, `popover`, and `spinner` primitives that fill genuine gaps. The `popover` registry item was verified project-aware to generate Base UI-only source with no Radix dependency and no package addition, so it is required; `chart` and `spinner` must still pass project-aware dry-run inspection before adoption. There is no existing showcase route.
...
- `public/appearance-bootstrap.js` for the synchronous same-origin bootstrap that reads the persisted preference before React hydration, resolves System with `matchMedia`, applies/removes the root `.dark` class, sets the resolved color scheme, and safely falls back to System when storage is unavailable or malformed. `src/features/design-system/lib/appearance.ts` owns the matching typed vocabulary and provider-side parser. This change adds no repository CSP because `script-src 'self'` alone would block Next.js inline framework/RSC scripts; effective deployed headers and the complete production runtime are a blocking implementation precondition.
...
- `src/features/design-system/components/appearance-menu-trigger.tsx` as the accessible client control with a ghost icon trigger, visible Light, Dark, and System radio labels, and dropdown semantics (mirrored through `appearance-menu-items.tsx`).
...
`src/components/layout/topbar.tsx` hosts the standalone appearance trigger: a ghost icon button opening a dropdown with the Light / Dark / System radio options, mirrored on public routes through `src/app/(public)/layout.tsx`. The avatar profile menu is identity and logout only. The shared authenticated `(app)` guard admits complete or deferred-enrollment active accounts under their one active account role; it does not create a role switch or change account state. When appearance is unavailable, the trigger renders nothing.

`resolveAppearanceAvailability` returns enabled for development and a valid dedicated demo deployment. Primary Production appearance is enabled only when the server-only `CLOIE_APPEARANCE_ENABLED` release setting is exactly `"true"`; unset, empty, `"false"`, malformed, and all other values remain disabled. Before task 9.1 readiness evidence is accepted, the setting remains unset in primary Production. The setting is listed in `.env.example` as a release control with an empty example value, never exposed through `NEXT_PUBLIC_*`, and does not affect the separately protected showcase route. Task 9.2 is an operator-only deployment action governed by `docs/runbooks/appearance-production-activation.md`; no repository commit activates it.

Before appearance implementation, the foundation ticket must inspect effective primary Production and dedicated-demo headers and confirm the selected same-origin external bootstrap works with hydration, client navigation, streamed RSC payloads, and a representative Server Action. This change does not introduce or weaken CSP. A conflicting host policy blocks implementation and requires an artifact revision. This validation is a blocking acceptance condition, not an open implementation detail.
...
The target set is `src/features/analytics/components/course-mean-pie-chart.tsx`, `faculty-cilo-analytics-chart.tsx`, `mean-bar-chart.tsx`, `stakeholder-mean-pie-chart.tsx`, and `qualitative-word-cloud.tsx`, plus their current consumers and dynamic loading fallback. The archived `improve-navigation-rendering-and-caching` change already established the aggregate-only client payload in commit `8e2582a`. This change starts chart work by running its serialization/access tests and preserves that contract. Detailed response-review routes retain their independently authorized raw-text behavior and are not repurposed as chart DTOs.
...
The approved palette has five categorical colors, whereas current pie/word-cloud data can exceed five categories. Repeated colors must receive deterministic marker, line, SVG pattern, or equivalent mark-level distinction and direct labels or visible legend text, never an expanded raw palette. Each chart exposes a concise textual insight and a tabular accessible alternative or summary. Server Components continue to prepare and authorize aggregate data; only Recharts/word-cloud rendering remains client-side.
...
src/features/design-system/
  ...
  components/
    ...
    appearance-menu-trigger.tsx
    appearance-menu-items.tsx
```
...
1. Design-system context, complete production-surface inventory, metadata, root bootstrap/provider, public shell, and shared primitive retokening. The global error surface remains owned by the later error slice.
...
3. Shared chart wrapper, quantitative chart conversion, and qualitative word-cloud conversion as separate slices, including accessible alternatives and existing visualization fallbacks.
4. Role navigation shells, profile/settings entry points, and common status/feedback patterns.
5. Secretary user management; Academic Calendar; Academic Structure; Course Assignments; enrollment management; Faculty profile/CILO authoring; and Dean oversight as separate domain slices.
...
7. Root landing/portal/public/onboarding; respondent response flow; respondent dashboard/profile/history; legal pages; and error/status pages as separate page-family slices.
...
- [A restrictive Content Security Policy blocks the bootstrap] → Confirm effective primary Production and dedicated-demo headers permit ADR 0010's same-origin external script before implementation. A conflict blocks work and requires reviewed artifacts; do not weaken policy or silently switch to nonce.
...
- [Chart migration reintroduces raw qualitative text in browser payloads] → Preserve commit `8e2582a`'s aggregate-only contract and run serialization/access tests before and after chart migration; chart payloads exclude raw text, response rows, identifiers, and emails.
...
4. Verify the completed aggregate-only Faculty analytics contract, then convert visualizations and add Recharts showcase coverage without broadening the payload.
...
- KPIs use the canonical shared Card composition in this change. No new `KpiCard` abstraction is introduced.
- Existing `sheet.tsx`, `tooltip.tsx`, and `radio-group.tsx` remain supported and receive showcase coverage. Unused-source removal requires a separate maintenance change.
- `tasks.md` is the authoritative exact production-surface ownership inventory. The readiness gate verifies that every inventory entry has a completed owner and fails on newly discovered unowned violations.

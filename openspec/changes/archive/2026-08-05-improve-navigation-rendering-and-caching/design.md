...
The first targets are Faculty Dashboard charts and the interactive Faculty Analytics flow. For Faculty Analytics, the server-authorized service `src/features/analytics/services/get-faculty-analytics-data.ts` and its Server Action `getFacultyAnalyticsDataAction` in `src/lib/actions/faculty-analytics-actions.ts` SHALL serialize only display metadata, quantitative aggregates, word-frequency tokens, response counts, and the server-computed qualitative answer-item count; the Server Action name and `evaluationIds` input contract remain unchanged while its returned projection is intentionally narrowed. The client-facing `FacultyAnalyticsData` type in `src/features/analytics/types.ts` SHALL drop the raw `qualitativeTexts` field and carry a server-computed `qualitativeItemCount` aggregate equal to the qualitative answer-item count across submitted responses, preserving the count the current cloud derives from `qualitativeTexts.length`. The route `src/app/(app)/faculty/analytics/page.tsx` keeps its existing Faculty role gate; the client selection state in `src/features/analytics/components/faculty-analytics-dashboard.tsx` and the qualitative cloud consumer in `src/features/analytics/components/faculty-qualitative-cloud.tsx` consume only the narrowed aggregates. A focused serialization regression test in `src/__tests__/modules/analytics-reporting-and-review/faculty-analytics-serialization.test.ts` SHALL assert the exact aggregate projection and exclude raw submitted qualitative response text, raw response rows, respondent identifiers, account emails, and undeclared summary fields. Detailed response-review routes remain independently authorized and are not repurposed as chart DTOs. `recharts`, word-cloud code, and drag-and-drop modules will not be moved into the persistent navigation shell. The production build remains a compilation gate; development seeded-account traces verify the route-local chunk boundaries selected for this change.
...
### 8. Verification is staged and environment-labeled

Each slice must leave focused automated proof and run existing quality gates. The repository currently has Vitest but no browser E2E or bundle-analysis setup. The change uses the smallest measurement support required to inspect authenticated route behavior; it does not add a general test platform without a dedicated decision.
...
- Browser network evidence showing server-rendered initial data or deferred client chunks as applicable, with the runtime mode stated explicitly.
...
Browser evidence for this change SHALL use the Prisma-seeded `@cloie.test` account catalog from `src/lib/constants/demo-users.ts` after an operator positively confirms that `DATABASE_URL`, `DIRECT_URL`, and the Supabase URL identify the intended primary development project, then runs `pnpm db:seed`. The route/rendering traces run with `pnpm dev` and the development-only role switcher backed by `cloie_dev_auth`; every record SHALL be labeled `development seeded-account route/rendering evidence`. This evidence verifies route rendering, server reads, hydration, navigation, and deferred payload behavior. It SHALL NOT be represented as production-build, OAuth exchange, callback, Supabase session-refresh, or deployment-performance evidence.

This change no longer depends on the dedicated demo deployment or `openspec/changes/add-dedicated-demo-auth/`. ADR 0008 remains accepted and continues to govern that separate deployment. The primary public Production deployment remains OAuth-only, and `cloie_dev_auth` plus `POST /api/auth/dev-login` remain unavailable after `pnpm build` and `pnpm start`.
...
Reconciliation evidence on 2026-08-05 used a 1440 by 900 viewport, Fast 3G, and 4x CPU throttling after a completed `pnpm db:seed`:

- Secretary Course Assignments rendered authorized records from the initial route response with no list `fetch` request. Development LCP was 4.647 seconds: 99 milliseconds TTFB and 4.547 seconds text render delay.
- Dean Dashboard used one document request plus scripts with no internal Dean API or `fetch` serialization. Development LCP was 3.285 seconds: 86 milliseconds TTFB and 3.199 seconds text render delay.
- Faculty Dashboard rendered heading and KPI content and requested Recharts and word-cloud chunks separately. Development LCP was 5.010 seconds: 57 milliseconds TTFB and 4.953 seconds text render delay.
- All three route snapshots had no browser console warnings or issues. The Faculty Dashboard Lighthouse snapshot scored 98 for accessibility and reported one residual heading-order defect: the `h1` is followed by the `Qualitative Response Insights` `h3`. This existing semantic defect is a follow-up candidate outside the payload/privacy reconciliation.

The development-only HMR, DevTools, and Turbopack scripts dominate these traces, so the timings SHALL NOT be compared with production performance budgets. Cache Components and offline behavior remain deferred.
...
1. Positively confirm that the configured database and Supabase URLs identify the intended primary development project, seed representative `@cloie.test` accounts with `pnpm db:seed`, then baseline route network and rendering behavior under `pnpm dev` through the development-only role switcher; label the record as development route/rendering evidence and exclude OAuth or deployment conclusions. Do not run the dedicated-demo isolation guard for this primary-development workflow because it requires and validates a different dedicated-demo project.

...
1. Seed representative `@cloie.test` accounts with `pnpm db:seed`, then baseline route network and rendering behavior under `pnpm dev` through the development-only role switcher; label the record as development route/rendering evidence and exclude OAuth or deployment conclusions.

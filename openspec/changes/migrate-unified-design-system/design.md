...
- `src/features/design-system/components/appearance-bootstrap-script.tsx` for the root bootstrap script that reads the persisted preference before React hydration, resolves System with `matchMedia`, applies/removes the root `.dark` class, sets the resolved color scheme, and safely falls back to System when storage is unavailable or malformed. Delivery is not preselected: the task 1.2 CSP feasibility decision determines whether the script ships inline under `'unsafe-inline'`, as a same-origin external bootstrap, under a hash-based SRI policy, or with a nonce propagated through `src/proxy.ts`, and that mechanism is recorded in ADR 0010 and this design before appearance implementation proceeds.

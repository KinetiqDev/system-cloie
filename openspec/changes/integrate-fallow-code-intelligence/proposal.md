...
- **Affected paths:** `.fallowrc.json`, `package.json`, new `.github/workflows/code-intelligence.yml`, committed Fallow baseline files, `AGENTS.md`, `docs/skills.md`, new Fallow operations documentation, and new project-level `opencode.json` MCP configuration. `.github/workflows/ci.yml` is inspected but remains unchanged. The existing untracked `.agents/skills/fallow/` is not an input to this change until its version alignment is explicitly resolved.

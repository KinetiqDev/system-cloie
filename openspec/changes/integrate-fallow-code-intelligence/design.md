...
On pull requests to `main`, the workflow verifies `github.event.pull_request.base.sha` is present, then runs `pnpm exec fallow audit --base "$BASE_SHA" --format json --quiet --sarif-file artifacts/fallow/audit.sarif > artifacts/fallow/audit.json` and captures the audit result without immediately ending the step. It uploads the JSON and SARIF artifacts with `actions/upload-artifact@v4` using `if: always()`, then propagates the saved result. Exit `1` is the intended unmatched error-severity quality-gate failure; exit `2` is a distinct execution/configuration failure. Full history makes the base diff and churn calculation deterministic.

...
`prisma/seed.ts` is 3,438 lines and mixes seed fixtures, database helpers, lifecycle-reset logic, phase runners, and process startup. This capability extracts cohesive seed-only modules without changing seeded database state or the `pnpm db:seed` entry point, making the seed deterministic, idempotent, and auditable.

# Course Roster Name Resolution

## Purpose

Define the authorized, bounded, preview-first use of official Student names to discover accounts for a Course roster without making names durable identity.
...
System CLOIE SHALL use Unicode NFKC normalization, Unicode-aware case-insensitive comparison, trimming, and repeated-whitespace collapse for strict comparison only. It SHALL preserve canonical `User.name` and uploaded source values and SHALL not persist a normalized roster name. Generic fuzzy similarity SHALL NOT prove account identity or produce an `EXACT_MATCH` or `SUGGESTED_MATCH`.
...
#### Scenario: Generic fuzzy similarity has no identity authority

- **GIVEN** an uploaded name has only a generic fuzzy similarity to a scoped account
- **WHEN** candidates are prepared
- **THEN** System CLOIE SHALL not classify that account as `EXACT_MATCH` or `SUGGESTED_MATCH` solely from that similarity
...
System CLOIE SHALL authorize the target Course assignment and resolve uploaded rows only within a bounded candidate population. Program-specific discovery SHALL require matching profile and active assignment-period placement Program but SHALL not exclude by assignment year level or section. General Education discovery SHALL accept any valid active placement in the assignment period.
...
- **THEN** the row SHALL be `AMBIGUOUS` and System CLOIE SHALL not select an account automatically
...
#### Scenario: A selected account is reused by another row

- **GIVEN** one source row already selects a candidate
- **WHEN** another source row selects that same candidate
- **THEN** System CLOIE SHALL classify the later row as `DUPLICATE_MATCH` and require remapping or skipping it before confirmation
...
A unique exact match MAY be ready without row acknowledgement. Suggested matches SHALL require one final acknowledgement of the current suggested-match count. Ambiguous, no-match, invalid-name, and `DUPLICATE_MATCH` rows SHALL be explicitly resolved or skipped; System CLOIE SHALL not silently omit them.
...
Preview, candidate search results, selections, skips, and final results SHALL exist only for the current management session. Closing a dirty preview SHALL require discard confirmation; closing or refreshing SHALL discard them.

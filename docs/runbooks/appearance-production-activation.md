...
This is the operator-facing procedure for activating the server-owned primary Production appearance release control once the design-system readiness evidence (issue [#225](https://github.com/Tugeru/project-cloie/issues/225)) is accepted. It complements [ADR 0010](../adr/0010-unified-appearance-and-protected-showcase.md). A Git commit documents this procedure but never performs its deployment action.

## Scope And Safety

- Perform this action only on the intended primary Production deployment after the readiness evidence recorded on issue [#225](https://github.com/Tugeru/project-cloie/issues/225) is accepted.
...
When disabled, the bootstrap forces Light before paint, ignores stored preferences, writes no preference, and omits the standalone appearance trigger in the app topbar and public layout. Safe Light tokens remain available during activation and rollback.
...
2. In a fresh browser context with OS Dark selected, verify the primary Production target remains Light before paint and has no appearance selector in the app topbar or public layout.
...
2. In fresh Light, Dark, and System browser contexts, verify the standalone appearance selector appears in the app topbar (and public layout) for an authenticated eligible account, and explicit Light/Dark/System behavior matches the accepted matrix.
...
3. In a fresh OS-Dark browser context, verify forced Light before paint and absence of the appearance selector in the app topbar and public layout.

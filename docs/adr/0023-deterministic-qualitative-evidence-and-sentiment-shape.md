...
**Shipped-state note (2026-09-11):** the deterministic core, both browser surfaces, both AI packets, and both prompts shipped together: `qualitative-analytics.ts` (analyzer, tone bands, prevalence), the Program Head qualitative view, the Faculty written-feedback view, the Program Head qualitative packet with budgeted `promptEvidence`, the Faculty packet with budgeted token and prompt tiers, the applied-filter facets on every packet, and prompt versions `program-head-analytics-v3` / `faculty-analytics-v6`. No database migration was required. Per-prompt evidence is keyed by evidence source, instrument version, and prompt label, and carries the instrument-version label; every prompt tier is character-budgeted so a broad scope degrades by omission instead of losing the whole interpretation.
...
The packet ceiling (`CLOIE_AI_MAX_PACKET_CHARS`) is a hard bound: a packet that still exceeds it after capping aborts the request as `unexpected`. Every tier is bounded before that check. The optional qualitative tiers degrade by omission, recording truncation so the browser can disclose what was analyzed — the Program Head packet in its `evidenceScope`, the Faculty packet in the per-tier `tokensTruncated`, `promptCountsTruncated`, and `promptTermsTruncated` flags that the Faculty AI evidence carries to the written-feedback view. Each tier spends only what the tiers before it left, so a broad scope still receives an interpretation. Ordered selection is deterministic: prompts by item count descending, terms by the prevalence order above.

Refinement (2026-09-11, Faculty surface). The Faculty packet originally serialized every participation, CILO, question, and trend row for the whole Faculty scope, so the deterministic base alone exceeded the ceiling on any real scope (42 CILO groups and 135 question groups measured at ~111,800 characters against a 16,000 ceiling) and the request aborted as `unexpected` before any provider call. The deterministic tiers are now capped by rows and by a per-tier share of the packet budget, carry clamped labels and rounded aggregates, and report each omission in a `truncations` list that reaches both the model's limitation and the browser (`evidence.truncatedEvidence`). Row selection is by rating volume descending, so a capped tier keeps its highest-volume evidence. Separately, the section bounds (observation, evidence length and count, connection, limitation, review question) moved out of the strict provider JSON schema and onto `normalizeInsightSection`, because a provider that validates its own generation against those bounds rejects the entire request when the model overflows one — surfacing to the reader as an intermittent unavailable insight. The section-capping rows of the packet remain per-evaluation: a capped question or CILO tier carries the highest-volume evaluation rows, and aggregating those tiers across evaluations of the same course remains a follow-up decision.

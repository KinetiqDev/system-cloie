...
Amended 2026-09-08: decision 5's post-publication roster lock is retired. Course rosters remain mutable while the Course assignment and Academic Period are active. Adding or restoring an eligible Student during an open published evaluation creates the missing EvaluationAssignment atomically with the membership write; removing a Student immediately removes access through dynamic roster eligibility while preserving any submitted response.
...
5. Permit ordinary roster writes after a Course-bound evaluation is published while the Course assignment and Academic Period remain active. Adding or restoring an eligible Student during an open published evaluation creates the missing EvaluationAssignment in the same transaction. Removing membership does not delete evaluation assignments or submitted evidence; the dynamic eligibility gate makes unsubmitted participation unavailable until membership is restored.
...
- Publication still snapshots the initial recipient set, exclusions, instrument structure, CILO bindings, and Course-assignment context. Later roster membership history and EvaluationAssignment timestamps provide the audit trail for late additions.

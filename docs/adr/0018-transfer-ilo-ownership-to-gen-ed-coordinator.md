...
Accepted — 2026-08-21 for Issue #490. (The associated OpenSpec change artifacts were later retired with the OpenSpec deprecation and are preserved in git history; see `docs/openspec-deprecation-migration-report.md`.)

Reconciles ADR 0005 and the General Education Coordinator deferred ownership conflict (also recorded in `src/features/outcomes/CONTEXT.md`).
...
- The earlier ILO work and ADR 0005 named the **Secretary** as ILO catalog owner (college-wide `code @unique`, `order`, `is_active`, timestamps).
- The live `src/features/outcomes/services/manage-outcome-writes.ts:243` denies Secretary ILO writes (`failure("You do not have permission ...")`).
- The General Education Coordinator role change added the `GEN_ED_COORDINATOR` role with shared college-wide `Course.course_scope == GENERAL_EDUCATION` scope but explicitly preserved the denial and added no ILO mutation path (deferred note previously in `src/features/outcomes/CONTEXT.md`).
...
3. **Transfer reconciles conflicting sources.** This ADR supersedes the deferred ownership claims in ADR 0005 §1 and the earlier ILO change for the ILO catalog encoder, and resolves the deferred notes (formerly in the retired OpenSpec config and in `src/features/outcomes/CONTEXT.md`) without silently toggling the write check. The live denial behavior is preserved and narrowed to Secretary while the Coordinator is allowlisted.
...
5. **College-wide scope, no portfolio table.** All Coordinators share the same `course.course_scope == GENERAL_EDUCATION` predicate college-wide. No per-Coordinator portfolio or assignment table is added; a partitioned scope requires a separately approved capability change.
...
- The General Education Coordinator boundary rules formerly carried in `openspec/config.yaml` are superseded by this ADR, the feature CONTEXT.md files, and `AGENTS.md`.
- `src/features/outcomes/CONTEXT.md` Secretary stewardship and deferred conflict sections are replaced with _General Education Coordinator outcome stewardship_ and a _resolved_ ownership note citing ADR 0018 and Issue #490.
...
- Retired change artifacts: the OpenSpec changes `introduce-institutional-learning-outcomes`, `add-general-education-coordinator`, and `transfer-ilo-catalog-to-gen-ed-coordinator` were retired with the OpenSpec deprecation (preserved in git history).
- Code: `src/features/outcomes/CONTEXT.md`, `src/lib/constants/navigation.ts:217`, `src/lib/constants/gen-ed-routes.ts`, `src/app/(app)/gen-ed-coordinator/layout.tsx:5`, Secretary `src/app/(app)/secretary/learning-outcomes/**/page.tsx`.

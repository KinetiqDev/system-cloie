...
This preserves the single-role account invariant from [ADR 0001 Single-Role Accounts](0001-single-role-accounts.md) (superseded by [ADR 0022](0022-multi-role-accounts-with-active-role-context.md); the no-default-program rule and per-request server validation stand) while making management scope deliberate and linkable. A primary/default Program field was rejected because the domain has no ranking rule and a fallback would recreate the authorization ambiguity this decision removes.

...
7. **1-to-1 enforcement migration:** the strict 1-to-1 link is enforced by a backfill (`course_assignment_id` populated from matching `[term_instance_id, course_id, faculty_id, section]`), then `ALTER COLUMN ... SET NOT NULL`, then a `UNIQUE INDEX` on `course_assignment_id`. Once enforced, the redundant legacy columns (`course_id`, `program_id`, `major_id`, `faculty_id`, `section`) on `CourseBoundEvaluation` are dropped in a separate cleanup migration.

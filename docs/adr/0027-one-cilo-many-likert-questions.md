...
The PLO side has no equivalent constraint: `itpqb_template_plo_question_key` keys the full `(template_id, plo_id, section_key, item_key)` tuple, so one PLO can already span several questions.
...
- A question still carries at most one CILO. Both binding tables keep their question-side unique key and widen the CILO-side key to `(…, cilo_id, section_key, item_key)`, matching the PLO key shape.
...
- [ADR 0025](0025-relax-central-deployment-plo-binding-gate.md) — the equivalent coverage-not-count rule on the Program-wide PLO side.

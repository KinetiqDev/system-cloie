...
| CILO ownership and lifecycle | "CILOs are encoded per course context, faculty-managed" (PRD §10.3; SRS §2.4.4, FR-3.3). CILOs are edited "for selected course contexts" by faculty. | ADR 0005 §3 (amended 2026-08-15): **CILOs are stored at the course level; stable across assignment periods; never assignment-specific or faculty-owned.** Owner = faculty for authoring; the CILO itself belongs to the course. Identity and Access glossary term `Course-level CILO` already encodes this. | wording | PRD/SRS describe faculty as course-context editors; ADR sharpens the lifecycle rule (course-level, not assignment-period-scoped, not faculty-owned). Rewrite must align with the existing `Course-level CILO` glossary entry. |
| CILO-to-GO mappings | Mapping is "supported" generically; faculty "map course outcomes to Graduate Outcomes where applicable" (PRD §8.3, §10.3; SRS FR-3.6). | ADR 0005 §4 (amended 2026-08-15): **Two typed relations — General Education CILO → Institutional Outcome; Program-specific CILO → owning-Program Graduate Outcome. Every active CILO needs at least one valid active target of its Course scope's typed layer for readiness and new publication.** | wording + blocking on the "at least one" rule | PRD/SRS never state the "every active CILO must have at least one valid active target" rule and never distinguish General Education from Program-specific alignment. ADR wins; rewrite must add the typed relations, the shared Course-level General Education mapping semantics, and the Faculty-primary / Program Head read-only mapping model. |
| Dean oversight of outcomes | "College Dean may review outcomes, governed stakeholder-tool templates, deployment context, analytics, reports, dashboards, and evidence outputs" (SRS §2.4.2). "College Deans shall have the same portal capabilities and operational access pattern as Program Heads" (PRD §6.2). PRD/SRS present Dean and Program Head as near-equivalent reviewers. | Map #103 + ADR 0005 §8 (amended 2026-08-15): **Dean has college-wide *read-only* oversight of the Institutional Outcome catalog, Program-specific GO coverage, and typed mapping gaps.** Dean overview presents Institutional Outcome coverage and General Education gaps before Program-specific GO coverage and gaps. The Dean-vs-Program-Head equivalence in the PRD/SRS is the *general academic-leadership portal* model; the outcome-ownership rule layer is a *read-only* subset, distinct from the operational equivalence. | wording | The PRD/SRS equivalence is not wrong for the academic-leadership portal surface, but it does not flag the read-only constraint on outcomes. Rewrite should add a Dean-outcomes read-only clause and typed ILO/GO coverage distinction. |
| Dean editing outcomes | Not explicitly granted, but the "same portal capabilities" wording is ambiguous. | ADR 0005 §8 (amended 2026-08-15): **Dean does not edit outcomes or mappings through this effort.** | wording | Rewrite must make the read-only constraint explicit. |
...
| Topic | PRD/SRS claim | Current decision | Severity | Notes |
|---|---|---|---|---|
| Account name source and shape | PRD/SRS do not specify whether account identity uses separate first/last fields, a canonical full name, or provider metadata authority. | ADR 0014: **`User.name` is one opaque canonical account name. Real Google OAuth supplies it for new accounts and first OAuth links; later callbacks preserve it; Secretary corrections remain authoritative. Development and dedicated-demo names remain fixture-controlled.** | open in PRD/SRS, resolved by current decision | The future PRD/SRS rewrite must remove any first-name/last-name assumption, state that the Gmail address never supplies the name, and distinguish the provisional pre-link name on Secretary-created accounts from the Google-derived name after first link. |
...
- **Account role completeness** — PRD/SRS use "invite" and treat Secretary as system administrator; `0001` makes Secretary-created accounts complete at creation time with no invitation workflow. ADR 0014 additionally makes the Secretary-entered account name provisional until first Google OAuth link, after which the Google-derived canonical name is preserved except for explicit Secretary correction.

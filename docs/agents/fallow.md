...
- The complete report runner (`pnpm fallow:reports`, script `scripts/run-fallow-reports.ts`, invoked post-merge, weekly, and manually by the CI workflow) writes `artifacts/fallow/<command>.json` and `<command>.sarif` for `dead-code`, `dupes`, `health`, and `flags`. Each command runs twice — `--format json --quiet` and `--format sarif --quiet` — because `--sarif-file` is a no-op for these commands in fallow 2.54.3; SARIF is captured from stdout instead.

...
- **Coupled Evidence Thresholds:** PH AI requires *both* sufficient quantitative responses and sufficient qualitative items before generating anything. A lack of written feedback blocks Outcomes AI, even if quantitative PLO evidence is robust.
...
- **Recharts Monopoly:** We will use the existing Recharts infrastructure and shared CLOIE chart primitives. No new charting libraries (e.g., D3, Nivo, Chart.js) will be introduced.
...
- **Outcomes AI:** Requires N quantitative ratings. Does *not* block on missing qualitative feedback.
...
*This slice proves the data model supports the intended analytics without touching the UI or AI contracts.*